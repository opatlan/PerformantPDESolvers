...
double* CN_Serial(double* A, double* B, double* C, double* nA, double* nB, double* nC, double* hX, double* Vmul, long long int N_S,double Int_Rate, double dt)
{
...
//int main(void) 
{
    ...
    double* nA = (double*)malloc(N_S * sizeof(*A));
    double* nB = (double*)malloc(N_S * sizeof(*B));
    double* nC = (double*)malloc(N_S * sizeof(*C));
    ...
    calc_coef(&S[1], A, B, C, nA, nB, nC, pbs_diff_eq);
    Vres = CN_Serial(A, B, C, nA, nB, nC, &hX[1], Vmul, N_S, Int_Rate,dt);
    t2 = clock();
